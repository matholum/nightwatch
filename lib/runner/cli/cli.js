...
class CliRunner {
  ...
  parallelMode(modules = null) {
    let testWorkerMode = this.test_settings.testWorkersEnabled && !this.isTestWorker();

    if (testWorkerMode && Array.isArray(modules) && modules.length <= 1) {
      return false;
    }

    return this.testEnvArray.length > 1 || testWorkerMode;
  }
  ...
  isConcurrencyEnabled(modules) {
    ...
    return this.parallelMode(modules);
  }
  ...
  runTests(done = null) {
    return this.runGlobalHook('before')
      ...
      .then(possibleError => {
        if (this.parallelMode() && Concurrency.isMasterProcess()) {
